...
class ProductController extends Controller
{
    ...
    {
        // Products data (you can replace this with actual data from DB or logic)
        $products = [
            1 => [
                'product_id' => 1,
                'product_name' => "AIR FORCE 1 '07 MEN'S BASKETBALL SHOES - WHITE",
            ...
            // Hardcoded stock data for each size variant
        $variants = [
            1 => [
                '35' => 0,
                '35.5' => 0,
        ...
        // Get the stock availability for the selected product
        $availableSizes = $variants[$id] ?? [];

        // Get the images for the selected product
        $imagesPath = public_path("images/products/{$id}");
        $images = File::files($imagesPath);

        // Check if there are images for this product
        $imageUrls = [];
        foreach ($images as $image) {
            $imageUrls[] = asset("images/products/{$id}/" . $image->getFilename());
        }

        // Return the product details view
        return view('cust.details', compact('product', 'imageUrls', 'availableSizes'));
    }
