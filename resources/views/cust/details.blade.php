        ...
        <div class="row">
            ...
            <div class="col-md-6" style="background-color: #f8f9fad8; padding: 20px;">
                ...
                <form action="{{ route('add_to_cart', $products->id) }}" method="POST" id="add-to-cart-form">
                    ...
                    <!-- Tombol Buy Now -->
                    <button type="submit" name="action" value="buy_now" class="btn w-100 mt-2" 
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        sizeRadios.forEach(function(radio) {
            radio.addEventListener('change', function() {
                if (this.checked) {
                    ...
                    handleSizeSelection(size, stock);
                }
            });
        });
        ...
        function handleSizeSelection(size, stock) {
        ...
        // Update hidden inputs for Buy Now form
        function updateBuyNowInputs() {
            const selectedSizeRadio = document.querySelector('input[name="size"]:checked');
            if (selectedSizeRadio) {
                buyNowSizeInput.value = selectedSizeRadio.value;
                buyNowQtyInput.value = qtySelect.value;
            } else {
                buyNowSizeInput.value = '';
        ...
        // Form validation before submit Add to Cart
        ...
        // Form validation before submit Buy Now
        document.getElementById('buy-now-form').addEventListener('submit', function(e) {
            if (!buyNowSizeInput.value) {
                e.preventDefault();
                alert('Please select a size before buying.');
                return false;
            }
            if (!buyNowQtyInput.value || buyNowQtyInput.value < 1) {
                e.preventDefault();
